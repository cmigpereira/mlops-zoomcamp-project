...
# pylint: skip-file
"""
This is based on the demo service for Evidently metrics integration with Prometheus and Grafana.

The service gets a reference dataset from reference.csv file and process current data with HTTP API.

The service also calculated the drift and sends alerts to the terminal.
...
class MonitoringService:
    ...
    def calculate(
        self, reference: str, production: pd.DataFrame, column_mapping: ColumnMapping
    ):
        """Add data to current dataset for specified dataset"""
        ...
        drift_ratio = n_drifted_features / n_features
        # display alerts if current drift is above threshold
        if drift_ratio > DRIFT_RATIO_THRESHOLD:
            logging.warning(
                f"\033[1;31;47m Beginning to detect drift: Drift ratio = {drift_ratio}"
            )
